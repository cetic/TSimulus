...
A time series essentially produces a sequence of values associated to equality spaced timestamps.
...
the underlying time series may produce such undefined values in a conservative way:

Transformation time series produce an undefined value if the underlying time series produces an undefined value.
The transformation applied may transform a defined value into an undefined one.

:ref:`aggregation` time series discard all the undefined values produced by the underlying time series, so that
