...
import be.cetic.tsimulus.timeseries.primary.dt.SecondTimeSeries
import spray.json.{DefaultJsonProtocol, JsObject, JsString, JsValue, _}

/**
  * A generator for [[be.cetic.tsimulus.timeseries.primary.dt.MillisecondTimeSeries]].
  *
  * @param name The generator name.
  */
class MillisecondTimeGenerator(name: Option[String]) extends Generator[Int](name, "ms")
{
   override def timeseries(generators: String => Generator[Any]) = SecondTimeSeries()

   override def toString = "MillisecondTimeGenerator()"

   override def equals(o: Any) = o match {
      case that: MillisecondTimeGenerator => that.name == this.name
      case _ => false
   }
...
object MillisecondTimeGenerator extends DefaultJsonProtocol with TimeToJson
{
   def apply(json: JsValue): MillisecondTimeGenerator = {
      ...
      new MillisecondTimeGenerator(name)
   }
}
