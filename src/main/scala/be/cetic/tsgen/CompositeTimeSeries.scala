...
/**
  ...
  */
class CompositeTimeSeries(val generators: Seq[TimeSeries[Double]]) extends TimeSeries[Double]
{
...
{
   def apply(generators: TimeSeries[Double]*): CompositeTimeSeries = new CompositeTimeSeries(generators)
}