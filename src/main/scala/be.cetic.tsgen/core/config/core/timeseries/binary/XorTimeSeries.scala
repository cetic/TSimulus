...
package be.cetic.tsgen.core.config.core.timeseries.binary

import be.cetic.tsgen.core.config.core.timeseries.TimeSeries
import org.joda.time.LocalDateTime

/**
  * A binary time series based on two time series. This time series is true iff one of the base time series is true,
  * and the other is false.
  *
  * If the value is not defined for at least one of the base time series, then the XOR value is not defined.
  */
case class XorTimeSeries(a: TimeSeries[Boolean], b: TimeSeries[Boolean]) extends TimeSeries[Boolean]
{
